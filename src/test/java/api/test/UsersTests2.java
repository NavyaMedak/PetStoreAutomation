...
public class UsersTests2 {
	...
	{
		Response response=UserEndPoints.createUser(userPalyload);
	...
	{
		Response response=UserEndPoints.readUser(this.userPalyload.getUsername());
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(),200);
	}
	...
	Response response=UserEndPoints.updateUser(this.userPalyload.getUsername(),userPalyload);
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(),200);
		
		//Checking data after update
		Response responseAfterupdate=UserEndPoints.readUser(this.userPalyload.getUsername());
		response.then().log().all();
	...
	{
		Response response=UserEndPoints.deleteUser(this.userPalyload.getUsername());
		Assert.assertEquals(response.getStatusCode(),200);
